...
def find_cars(img, model, X_scaler, y_start_stops, scales, window, color_space, spatial_size, hist_bins, orient, pix_per_cell, cell_per_block, hog_channel, spatial_feat, hist_feat, hog_feat):
    ...
    for scale, (y_start, y_stop) in zip(scales, y_start_stops):
        ...
        if scale != 1:
            ...
            img_tosearch = cv2.resize(img_tosearch, (np.int_(imshape[1]/scale), np.int_(imshape[0]/scale)))
